...
import MSNGenerator from '../components/MsnGenerator'

const FAKE_FRIENDS = [
  { name: 'Gutinho', color: '#C8A8F8' },
  { name: 'Fabinha', color: '#A8C8F8' },
  { name: 'Maria', color: '#F8C8A8' },
  { name: 'Oscar', color: '#A8F8C8' },
  { name: 'Miloca', color: '#F8A8C8' },
  { name: 'Ma', color: '#C8F8F8' },
  { name: 'João', color: '#F8F0A8' },
  { name: 'Lucas', color: '#D8C8F8' },
]

const FAKE_SCRAPS = [
  { from: 'Maria', msg: 'Feliz aniversário linda!! ✨', time: 'há 2 min' },
  { from: 'Mariana', msg: 'Muitas felicidades pra você!! 🎉', time: 'há 5 min' },
  { from: 'Lucas', msg: 'Parabéns!! Que venham muitos mais!', time: 'há 12 min' },
]
...
export default function OrkutPage({ initialTestimonials }: Props) {
  ...
  const TABS = [
    { key: 'depoimentos' as const, label: `depoimentos (${testimonials.length})` },
    { key: 'recados' as const, label: `recados (${FAKE_SCRAPS.length})` },
      ...
      <div className="main-wrapper">
        ...
        <div className="content-layout">
          ...
          <div className="main-content">
            {/* Tabs */}
            <div style={{ display: 'flex', gap: '2px', marginBottom: '6px', borderBottom: '2px solid #6A5ACD', flexWrap: 'wrap' }}>
              {TABS.map(tab => (
                <button
                  key={tab.key}
                  onClick={() => {
                    ...
                    color: activeTab === tab.key ? 'white' : '#4B3BA8',
                  }}
                >
                  {tab.label}
                </button>
            ...
            {activeTab === 'extras' && (
              <>
                <div className="panel" style={{ marginBottom: '8px' }}>
                  <div className="section-header">💘 teste de compatibilidade com ruth</div>
                  <CompatibilityQuiz />
                </div>
                <div className="panel">
                  <div className="section-header">💬 gerador de print de msn messenger</div>
                  <MSNGenerator />
                </div>
              </>
            )}
